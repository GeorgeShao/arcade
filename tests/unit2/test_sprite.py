...
class MyTestWindow(arcadeplus.Window):
    ...
    def __init__(self, width, height, title):
        ...
        sprite = arcadeplus.Sprite(":resources:images/items/coinGold.png", CHARACTER_SCALING)
        sprite.position = (130, 130)
        sprite.set_position(130, 130)
        sprite.angle = 90
        self.coin_list.append(sprite)

        self.individual_coin = arcadeplus.Sprite(":resources:images/items/coinGold.png", CHARACTER_SCALING)
    ...
    def on_draw(self):
        try:
            ...
            assert arcadeplus.get_pixel(150, 50) == (191, 121, 88)

            assert arcadeplus.get_pixel(230, 230) == (59, 122, 87)
            self.individual_coin.draw()
            assert arcadeplus.get_pixel(230, 230) == (255, 204, 0)

            # Test for coin scaling
            if self.frame_counter < 5:
                assert arcadeplus.get_pixel(130, 150) == (59, 122, 87)
            else:
                assert arcadeplus.get_pixel(130, 150) == (227, 182, 2)
    ...
    def update(self, delta_time):
        ...
        coin_hit_list = arcadeplus.check_for_collision_with_list(self.character_sprite, self.coin_list)
